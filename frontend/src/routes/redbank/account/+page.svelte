<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    successMessage = '';
    try {
      const response = await fetch(`http://localhost:8000/_api/redbank/account/${inputString}`, {
        method: 'GET',
        headers: {
  ...
  async function handleDeposit() {
    ...
    successMessage = '';
    try {
      const response = await fetch('http://localhost:8000/_api/redbank/deposit', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  async function handleTransfer() {
    ...
    successMessage = '';
    try {
      const response = await fetch('http://localhost:8000/_api/redbank/transfer', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
...
<div class="container">
<h1>🔴 RedBank Account Information</h1>
