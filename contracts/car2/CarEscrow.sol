...
contract CarEscrow {
    ...
    event CarOwnerTransfer(address buyer, address seller, Car car, uint256 price);
    event WithdrawnDeposit(address buyer, uint256 amount);
    event ContractExpired(address buyer, address seller, Car car);
    ...
    modifier hasSufficientBalance(uint256 amount) {
        require(
            ...
            "CarEscrow: the buyer does not have sufficient balance of tokens to make deposit"
        );
        _;
    }
    ...
    function _executeDeal() internal {
        ...
        emit CarOwnerTransfer(_buyer, _seller, _car, _price);
    }
    ...
    /**
     * @dev Get the time left until the contract expires.
     */
    function timeUntilExpiry() external returns (uint256) {
        if (block.timestamp > _depositReleaseDate) {
            emit ContractExpired(_buyer, _seller, _car);
            return 0;
        }

        return _depositReleaseDate - block.timestamp;
    }
}
